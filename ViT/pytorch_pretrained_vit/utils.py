...
def remap_for_parameter_only(ckpt, target_dtype=None, device=None):
    ...
    patterns = [
        (re.compile(r"\.proj_q\.weight$"),   ".proj_q"),
        (re.compile(r"\.proj_q\.bias$"),   ".proj_q_bias"),
        (re.compile(r"\.proj_v\.weight$"), ".proj_v"),
        (re.compile(r"\.proj_v\.bias$"), ".proj_v_bias"),
        (re.compile(r"\.proj_k\.weight$"), ".proj_k"),
        (re.compile(r"\.proj_k\.bias$"), ".proj_k_bias"),
    ]
...
def load_pretrained_weights(
    ...
    if weights_path is None:
        ...
        if is_quantization == False:
            state_dict = torch.load(weights_path)
        else:
            state_dict = torch.load(weights_path)
            state_dict = remap_for_parameter_only(state_dict)
